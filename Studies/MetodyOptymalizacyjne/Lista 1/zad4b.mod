...
param AllowedDays{Days} within Choice, default 0;  /*  allowed days for academical activities   */
param MinimalRank, >= 0;                           /*  minimal allowed rank of each academic group  */
...
/*  ensures that academical activities take place only in allowed days  */
s.t. activitiesOnlyInAllowedDays{day in Days: AllowedDays[day] == 0}:
    sum{hour in Hours: DaysToHours[hour, day] = 1}
        sum{subject in Subjects, group in Groups: PlanAllocation[hour, group, subject] = 1}
            signed[group, subject]
    = 0;

/*  ensures that student is signed up only to groups with rank 5 or higher  */
s.t. allGroupsWithMinimalRank{subject in Subjects, group in Groups: Ranks[group, subject] < 5}:
    signed[group, subject] = 0;
